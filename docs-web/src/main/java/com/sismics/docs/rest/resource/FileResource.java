...
import java.io.OutputStream;  // 对应 OutputStream
import java.io.StringReader;  // 对应 StringReader
import java.net.HttpURLConnection;  // 对应 HttpURLConnection
import java.net.URL;  // 对应 URL
...
public class FileResource extends BaseResource {
    ...
    public Response add(
        ...
        try {
            ...
            // Now process the file with DeepSeek API for tags and summarization
            processFileWithDeepSeek(unencryptedFile);
    ...
    // 额外添加的方法：检查文件是否为 PDF
    private boolean isPdfFile(java.nio.file.Path file) {
        String fileName = file.getFileName().toString();
        return fileName.endsWith(".pdf");
    }

    // 额外添加的方法：从 PDF 提取文本内容
    private String extractTextFromPdf(java.nio.file.Path pdfFile) {
        try {
            ...
            throw new ServerException("PdfProcessingError", "Error extracting text from PDF", e);
        }
    }
    ...
    private void processFileWithDeepSeek(java.nio.file.Path unencryptedFile) {
        try {
            ...
            String apiKey = "sk-bf8854c5d5814b66a58b3cd2d18e2121"; // 建议改为从配置读取

            // 使用JSON库构建请求体
            String prompt = "请为以下文档生成标签（用逗号分隔）和摘要，按此格式返回：\n"
                    + "Tags: 标签1, 标签2\n"
                    + "Summary: 这里是摘要\n"
                    + "文档内容：\n" + fileContent;
            ...
            String response = callDeepSeekApi(apiUrl, apiKey, requestBody.toString());
    ...
    private String callDeepSeekApi(String apiUrl, String apiKey, String requestBody) {
        try {
            ...
            if (statusCode >= 400) {
                String errorMessage = "API请求失败: " + statusCode + " - " + connection.getResponseMessage();
            ...
            throw new ServerException("ApiCallError", "API调用失败: " + e.getMessage(), e);
        }
    }
...
/**
 * Trigger a download for the generated file.
 * @param filePath Path to the file
 */
private void triggerFileDownload(java.nio.file.Path filePath) {
    try {
        // This would trigger a file download on the client side.
        // The specifics depend on your frontend implementation or API handling.
        // For example, sending a response with the file data to the browser for download.
    } catch (Exception e) {
        throw new ServerException("FileDownloadError", "Error triggering file download", e);
    }
}
